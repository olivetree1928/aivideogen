...
export async function insertUser(user: User) {
  ...
  const res = await db.query(
    `INSERT INTO imaginevisual_users 
...
export async function getByEmail(email: string): Promise<User | undefined> {
  const db = getDb();
  const res = await db.query(`SELECT * FROM imaginevisual_users WHERE email = $1 LIMIT 1`, [
    email,
  ]);
  if (res.rowCount === 0) {
    return undefined;
...
export async function getByUuidAndEmail(uuid: string, email: string) {
  const db = getDb();
  const res = await db.query(
    `SELECT * FROM imaginevisual_users WHERE uuid = $1 AND email = $2 LIMIT 1`,
    [uuid, email]
  );
  return formatUser(res.rows[0]);
}

export function formatUser(row: QueryResultRow): User | undefined {
  if (!row) {
    return undefined;
  }
