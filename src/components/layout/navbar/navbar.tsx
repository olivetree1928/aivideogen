  ...
  ({ classNames = {}, ...props }, ref) => {
    ...
    return (
      <Navbar
        ...
        {...props}
        classNames={{
          base: cn("border-default-100 bg-transparent text-foreground"),
          wrapper:
      ...
      >
        ...
        <NavbarContent className="hidden md:flex" justify="center">
          <NavbarItem onClick={() => handleTagClick("home")}>
            <Link
              aria-current="page"
              className={cn(
                "text-foreground mx-4",
                activeTag === "home" ? "text-foreground font-bold" : ""
              )}
              href={`/${locale}`}
              size="md"
            >
          ...
          <NavbarItem onClick={() => handleTagClick("text-to-image")}>
            <Link
              className={cn(
                "text-foreground mx-4",
                activeTag === "text-to-image" ? "text-foreground font-bold" : ""
              )}
          ...
          <NavbarItem onClick={() => handleTagClick("pricing")}>
            <Link
              className={cn(
                "text-foreground mx-4",
                activeTag === "pricing" ? "text-foreground font-bold" : ""
              )}
              href={`/${locale}/pricing`}
              size="md"
            >
          ...
          {user ? (
            <div className="flex flex-row gap-2">
              <button className="flex justify-center items-center gap-3 mr-6 hover:scale-110 transition-all duration-300">
                <a href={`/${locale}/dashboard`} className="text-foreground">
                  My creations
                  {/* <Icon
                    icon="lucide:settings-2"
                    className="w-[1.3em] h-[1.3em] text-foreground"
                  /> */}
                </a>
              </button>
              <UserButton />
        ...
        <NavbarMenu
          className="top-[calc(var(--navbar-height)_-_1px)] max-h-fit bg-white dark:bg-black pb-6 pt-6 shadow-medium backdrop-blur-md backdrop-saturate-150 text-foreground mx-1"
          motionProps={{
            ...
            {[
              ...
              >
                <Link
                  className={cn(
                    "text-foreground",
                    activeTag === tag ? "text-foreground font-bold" : ""
                  )}
                  href={`/${locale}/${path}`}
                  size="lg"
                  onClick={() => {
