      ...
      <Tabs
        classNames={{
          ...
          tab: [
            ...
            "data-[selected=false] > * text-gray-900 dark:text-white",
            "px-2 sm:px-4", // Add padding that's smaller on mobile
          ].join(" "),
          tabContent:
            "group-data-[selected=true]:text-white group-data-[selected=false]:text-gray-900 dark:text-white text-xs sm:text-base", // Make text smaller on mobile
        }}
        radius="full"
        color="secondary"
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mx-8 md:mx-0">
        {tiers.map((tier) => (
          <Card
            key={tier.key}
            isBlurred
            className={cn("bg-white dark:bg-gray-900 p-3 border-1 border-blue-300 dark:border-gray-600", {
              "border-blue-500 dark:border-blue-400": tier.mostPopular,
            })}
            shadow="md"
          >
            {tier.mostPopular ? (
            ...
            <CardHeader className="flex flex-col items-start gap-2 pb-6">
              <h3 className="text-large font-medium text-gray-900 dark:text-white">{tier.title}</h3>
            ...
            <CardBody className="gap-8">
              <p className="flex items-baseline gap-1 pt-2">
                ...
                <span className="text-4xl font-semibold leading-7 tracking-tight text-gray-900 dark:text-white">
                  {typeof tier.price === "string"
                    ? tier.price
                    : tier.price[selectedFrequency.key]}
                </span>
                {typeof tier.price !== "string" ? (
                  <span className="text-small font-medium text-gray-500 dark:text-gray-400">
                    {tier.priceSuffix
