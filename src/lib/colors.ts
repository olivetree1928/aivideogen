...
// 应用CSS变量到根元素
export function applyTheme(theme: ColorTheme, themeKey?: string) {
  if (typeof document === 'undefined') return;
  
  const root = document.documentElement;
  
  console.log('Applying theme:', theme.name, 'with key:', themeKey, theme.colors);
  
  // 设置或移除dark类名
  if (themeKey === 'dark' || themeKey?.includes('dark')) {
  ...
  console.log('Applied theme:', theme.name, 'with CSS variables:', {
